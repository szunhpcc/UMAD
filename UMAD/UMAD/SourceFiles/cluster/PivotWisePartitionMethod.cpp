...
CPartitionResults PivotWisePartitionMethod::partition(CMetricDistance *metric, const vector<shared_ptr<CMetricData> > &pivots,vector<shared_ptr<CMetricData> > &data,int first,int size,int numPartitions,int maxLeafSize,double trisectionRadius,char* selectOptimalPivots,char* partitionByOnePivot)
{
	...
	{
		partitonCase = shared_ptr<PartitionByOnePivot>(new PartitionByOnePivot_Balance());
	}
	else if(!strcmp(partitionByOnePivot,"Kmeans"))
	{
		partitonCase = shared_ptr<PartitionByOnePivot>(new PartitionByOnePivot_Kmeans());
	}
	else if(!strcmp(partitionByOnePivot,"DBSCAN"))
	{
		partitonCase= shared_ptr<PartitionByOnePivot>(new PartitionByOnePivot_DBSCAN());
	}
	else if(!strcmp(partitionByOnePivot,"Trisection"))
	{
		partitonCase= shared_ptr<PartitionByOnePivot>(new PartitionByOnePivot_Trisection(trisectionRadius));
	}
	else
	{
		partitonCase= shared_ptr<PartitionByOnePivot>(new PartitionByOnePivot_BalanceOne());
	}
	do
	{
		...
		for(int i=0; i< taskList.size();++i)
		{
			...
			if(!taskList[i]->getIsDeal() && taskList[i]->getData()->size()>maxLeafSize && levelNum < newPivots.size() && taskList[i]->getData()->size() < size)
			{
				task = taskList[i];
				break;
			}
			else if((!taskList[i]->getIsDeal() && taskList[i]->getData()->size() <= maxLeafSize)||levelNum == pivots.size() || taskList[i]->getData()->size() >= size)
			{
	...
	}while((taskList.size() !=1) && ((tasklist_indictor < taskList.size()-1) || ((tasklist_indictor == taskList.size()-1) && task != 0 && !task->getIsLeafNode())));
