...
void batchBulkLoad(char *&dataFileName,int &numPivot,int setA,int setN,int &singlePivotFanout,int fftscale,char *&pivotSelectionMethod,char *&partitionMethod,int &maxLeafSize,char *&dataType,int initialSize,int finalSize,int stepSize,bool bucket,int &fragmentLength,int dim,double maxR,char *&indexType,int fftOpt,char *buildingInformationFile,int buildMode,char *indexName,double MF_maxRadius,double MF_middleProportion,bool putInfo,int bThreadNum,double trisectionRadius,char* selectOptimalPivots,char* partitionByOnePivot)
{
	...
	for (int size = initialSize, i = 0; (size <= finalSize) & (i < indexNum); size += stepSize, i++)
	{
		...
		if(putInfo==1)
			getMVPIndexStructureInfo(newIndexName,dataType,buildingInformationFile);
...
void searchIndex(int dataBasehSize,char* queryFileName,char *dataType,int firstQuery,int lastQuery,int dim,int fragmentLength,double maxRadius,double minRadius,double step,bool verify,char *resultsFileName,int buildMode,char *indexName,int searchMode,int cacheHeight,int sThreadNum,bool cacheAll,int numberOfChildrenPathSearchingThreads)
{
...
#ifdef _WIN32
	char *fileName="./SourceFiles/util/database/";
#else
	char *fileName="../database/";
#endif
	joinCharArray(fileName,indexName);
	...
	_search(queryList,cmetric,resultFile,fileName,newQueryFile,dim,fragmentLength,verify,dataType,maxRadius,minRadius,step,searchMode,firstQuery,lastQuery,dataBasehSize,cacheHeight,cacheAll,sThreadNum,numberOfChildrenPathSearchingThreads);
...
void _search(vector<shared_ptr<CMetricData>> *queryList,CountedMetric *cmetric,char *resultFileName,char *indexFileName,char *rawDataFile,int dim,int fragmentLength,bool verify,char *dataType,double maxRadius,double minRadius,double step,int searchMode,int firstQuery,int lastQuery,int dataBaseSize,int cacheHeight,bool cacheAll,int maximumThreadsNumForSearching,int totalSearchingThreadForChildPathSearchingTaskInThreadpool)
{
	...
	else
	{
		...
		vector<shared_ptr<CMetricData> >::iterator ite=queryList->begin();
		newQueryList.insert(newQueryList.begin(),ite+queryStart,ite+queryEnd);
		multiThreadsInInterAndInner(&newQueryList,dataType,indexFileName,maxRadius,minRadius,step,resultFileName,dataBaseSize,cacheHeight,maximumThreadsNumForSearching,totalSearchingThreadForChildPathSearchingTaskInThreadpool);
	}
}
...
{	
	...
	long *resultCounter=new long[runNumber];
