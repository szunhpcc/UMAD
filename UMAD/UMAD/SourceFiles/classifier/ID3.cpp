#include "../../HeaderFiles/classifier/ID3.h"

//根据数据实例计算属性与值组成的map  
void CID3::ComputeMapFrom2DVector()
{  
    unsigned int i,j,k;  
    bool exited = false;  
...
//根据具体属性和值来计算熵  
double CID3::ComputeEntropy(vector <vector <string> > remain_state, string attribute, string value,bool ifparent)
{  
...
double CID3::ComputeGain(vector <vector <string> > remain_state, string attribute)
{  
    unsigned int j,k,m;  
    //首先求不做划分时的熵  
...
int CID3::FindAttriNumByName(string attri)
{  
...
//找出样例中占多数的正/负性  
string CID3::MostCommonLabel(vector <vector <string> > remain_state)
{  
    int p = 0, n = 0;  
...
//判断样例是否正负性都为label  
bool CID3::AllTheSameLabel(vector <vector <string> > remain_state, string label)
{  
    int count = 0;  
...
//返回根结点指针  
Node * CID3::BulidDecisionTreeDFS(Node * p, vector <vector <string> > remain_state, vector <string> remain_attribute)
{  
...
void CID3::PrintTree(Node *p, int depth)
{  
